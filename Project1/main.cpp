...
public: 
	...
	Matrix(int _str, int _st) //str - строки, st - столбцы
	{
		...
		for (int i = 0; i < str; i++)
		{
			ptr[i] = new T[st](); // Цикл for инициализирует каждую строку матрицы. 
			                      //Для каждой строки создаётся новый массив типа T размером st,
			                      //который инициализируется значениями по умолчанию
		}
